...
@register("dynamic_ablation")
class DynamicAblationRewardManager(AbstractRewardManager):
    """The reward manager."""
        ...
        for q in unique_questions:
            ...
            if acc_q>0:
                ...
                for i in group_indices:
                    L = completions_length[i]
                    wrong_length+=L
                avg_q_len_wrong=wrong_length/len(group_indices)
                avg_q_len=0
            ...
            for i in group_indices:
                ...
                if acc_q >= threshold: ## threshold = 0.2
                    # z_i = (L_i - avg_q_len) / max_response_len #0.02左右
    ...
    def __call__(self, data: DataProto, global_steps: int = 0,return_dict: bool = False) -> torch.Tensor | dict[str, Any]:
        ...
        # if self.num_examine==0: #训练集
        #     # 记录长度和奖励统计
        #     import wandb
        #     # print("rewards:",rewards)
        #     if wandb.run is not None:
        #         wandb.log({
        #             "reward/length_penalty": sum(length_penalty)/len(length_penalty),
        #             "reward/length_reward": sum(length_reward)/len(length_reward),
        #             "reward/length_exceed_penalty": sum(length_exceed_penalty)/len(length_exceed_penalty),
        #             "reward/outcome_acc": sum(rewards) / len(rewards)
