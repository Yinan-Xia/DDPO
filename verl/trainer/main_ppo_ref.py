...
Failed_RESULTS_DIR = "failed_results_32b"
Correct_RESULTS_DIR = "correct_results_32b"

RESULTS_DIR = "problem_correctness_32b"
...
def cal_acc(token_level_rewards: torch.Tensor,
            index: torch.Tensor,
            input_ids,
            ground_truth,
            completions_length,
    ...
    with torch.no_grad():
        ...
        for index_i in id2correct_num:
            ...
            group_indices = [i for i, q_idx in enumerate(index) if q_idx == index_i]
            L=0
            for i in group_indices:
                L += completions_length[i]
            avg_L = L/len(group_indices)
            length_i = completions_length[index_i]
            ...
            datas_to_save[index_i]=data_to_save
...
class RewardManager():
    ...
    def get_reweight_rewards_pre(self, rewards, completions_length, index):
            """
    ...
    def get_reweight_rewards(self, rewards, completions_length, index):
        ...
        index: list indicating which question each sample belongs to
        """
        id2correct_ratio = cal_acc(rewards,index)
        ...
        for q in unique_questions:
            ...
            for i in group_indices:
                ...
                epsilon = 1e-6
                z_i = (L_i - avg_len) / (std_len + epsilon)
                if acc_q>threshold: ## threshold待设置
                    rewards[i] = rewards[i] - alpha * z_i
                else:
                    rewards[i] = rewards[i] + alpha * z_i
                # if rewards[i] > 0:
                #     L_i = completions_length[i]
    ...
    def __call__(self, data: DataProto):
        ...
        if self.test:
            ...
            id2correct = cal_acc(rewards, index,input_ids,ground_truth,completions_length)
