...
# RESULTS_DIR = "problem_correctness_qwen3_14b_dynamic_grpo"
...
def compute_grpo_outcome_advantage(
    ...
    with torch.no_grad():
        ...
        for i in range(bsz):
            ...
            id2length[index[i]].append(completions_length[i])
            id2bsz_i[index[i]].append(i)
            if index[i] not in id2input:
                id2input[index[i]] = input_ids[i]
                id2ground_truth[index[i]] = ground_truth[i]
                id2id[index[i]] = extra_ids[i]
        for idx in id2score:
            if len(id2score[idx]) == 1:
                id2mean[idx] = torch.tensor(0.0)
                id2std[idx] = torch.tensor(1.0)
        ...
        RESULTS_DIR = os.path.join("/share/xiayinan/verl/trainset_correctness",config.trainer.experiment_name)
        file_name = f"{RESULTS_DIR}/correctness_{step}.json" 
        timestamp = datetime.now().strftime("%Y%m%d_%H%M%S_%f")
        # print(f"DEBUG: Algorithm estimator check passed")
        # print(f"DEBUG: config.trainer.experiment_name = {config.trainer.experiment_name}")
        # print(f"DEBUG: RESULTS_DIR = {RESULTS_DIR}")
        # print(f"DEBUG: file_name = {file_name}")
        # print(f"DEBUG: id2correct_num keys = {list(id2correct_num.keys())}")
        ...
        for index_i in id2correct_num:
            ...
            length = sum(id2length[index_i]) / len(id2length[index_i])
            data_to_save = {
                ...
                "length":length.tolist() if torch.is_tensor(length) else length,
                "correctness": sum(id2correct_num[index_i]) / group_size
            }
            # Convert tensor key to string for JSON serialization
            key = str(int(id2id[index_i])) if torch.is_tensor(id2id[index_i]) else str(id2id[index_i])
            datas_to_save[key] = data_to_save
            # 过滤easy样本
            filter_easy_acc = config.trainer.filter_easy_acc
            filter_hard_acc = config.trainer.filter_hard_acc
            acc = sum(id2correct_num[index_i]) / group_size
            if  not filter_hard_acc <= acc <= filter_easy_acc: #暂时设Threshold_easy为0.8
                print(f"DEBUG: change the response_mask, acc is {sum(id2correct_num[index_i]) / group_size}")
                idx = id2bsz_i[index_i]
                response_mask[torch.tensor(idx)]=0
        # with open(file_name, "w", encoding="utf-8") as f:
        #     json.dump(datas_to_save, f, ensure_ascii=False, indent=4)
        #     print(f"DEBUG: file has been saved to {file_name}")
        # print(f"DEBUG: is file exists: {os.path.exists(file_name)}")
