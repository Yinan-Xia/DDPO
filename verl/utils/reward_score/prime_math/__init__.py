...
import re
import sys
import multiprocessing  # 必须在SymPy前导入
import sympy
...
def are_equal_under_sympy(ground_truth_normalized: str, given_normalized: str):
    ...
    try:
        ...
        if should_allow_eval(expr):
            ...
            simplified = sympy.simplify(sympy_diff) # 这步有问题
            if simplified == 0:
                are_equal = True
    except Exception:
        pass
    return are_equal
    # 1. 直接字符串比较
    # if ground_truth_normalized.strip() == given_normalized.strip():
    #     return True

    # # 2. 数值比较（处理 13/2 vs 6.5 这种情况）
    # try:
    #     # 尝试解析为数值
    #     val1 = eval(ground_truth_normalized.replace('^', '**'))
    #     val2 = eval(given_normalized.replace('^', '**'))
    ...
    # import re
    # # 检查是否是简单分数形式 a/b
    # frac_pattern = r'^-?\d+\.?\d*/\d+\.?\d*$'
    # if re.match(frac_pattern, ground_truth_normalized.strip()) and re.match(frac_pattern, given_normalized.strip()):
    #     try:
    #         from fractions import Fraction
    #         f1 = Fraction(ground_truth_normalized.strip())
    #         f2 = Fraction(given_normalized.strip())
    #         return f1 == f2
    #     except:
    #         pass

    # # 4. 最后才使用SymPy
    # return are_equal_under_sympy_original(ground_truth_normalized, given_normalized)
